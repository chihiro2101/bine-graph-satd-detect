...
class Graph(defaultdict):
  ...
  def random_walk_restart_for_large_bipartite_graph(self, nodes, percentage, alpha=0, rand=random.Random(), start=None):
    """ Returns a truncated random walk.
        ...
        start: the start node of the random walk.
    """
    G = self #graph ({'u5998': ['i10'], 'u5999': ['i10', 'i653'], 'u6000': ['i10']})
    if start: #the start node of the random walk ex: start = 'u0'
      path = [start]
    else:
      # Sampling is uniform w.r.t V, and not w.r.t E
      path = [rand.choice(nodes)] #random node for starting
    # import pdb;pdb.set_trace()
    while len(path) < 1 or random.random() > percentage:
      cur = path[-1] # phần tử cuối của path
      neighbors = set([])
      # import pdb;pdb.set_trace()
      for nei in G[cur]: #duyệt các item nodes có liên kết vs node user cur
          neighbors = neighbors.union(set(G[nei])) #union set neighbors của từng item node (user node)
      # print(len(neighbors))
      neighbors = list(neighbors) #neighbors là những user cùng rate một/nhiều items
      # import pdb;pdb.set_trace()
      if len(G[cur]) > 0:
        if rand.random() >= alpha:
          add_node = rand.choice(neighbors)
    ...
    return path #['u0', 'u551', 'u306', 'u767', 'u470', 'u422', 'u63']
    # path: ['i1146', 'i1181', 'i1197', 'i577', 'i295', 'i327', 'i110', 'i83', 'i81']
    # path: ['i1146']
    # path: ['i1146', 'i1181', 'i10']
...
def build_deepwalk_corpus_random_for_large_bibartite_graph(G, hits_dict, percentage, maxT, minT, alpha=0, rand = random.Random(), node_type='u'):
  walks = []
  nodes_total = list(G.nodes()) #['u0', 'i0', 'i408', 'u1', 'i1442', 'i515', 'i1161']
  nodes = [] #list of user node
  # import pdb;pdb.set_trace()
  # print(len(nodes),node_type)
  for obj in nodes_total:
    # import pdb;pdb.set_trace()
    if obj[0] == node_type: #obj = u5999, obj[0] = u, node_type = u
      nodes.append(obj)
      # print("obj = {}".format(obj))
      # print("obj[0] = {}".format(obj[0]))
...
def load_edgelist(file_, undirected=True):
  ...
  #adddddd
  # import pdb;pdb.set_trace()
  file_ = os.path.join('data', file_.split("../")[1])
