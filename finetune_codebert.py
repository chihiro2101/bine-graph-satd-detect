...
from ignite.metrics import Accuracy, Loss, ClassificationReport, ConfusionMatrix
import numpy as np
import os
...
parser.add_argument('--dataset', default='SATD')
parser.add_argument('--bert_init', type=str, default='microsoft/codebert-base',
                    choices=['roberta-base', 'roberta-large', 'bert-base-uncased', 'bert-large-uncased','microsoft/codebert-base'])
...
# bert_init = 'microsoft/codebert-base'
...
def train_step(engine, batch):
    ...
    # label_list = y_true.clone().detach()
    # df = pd.DataFrame(label_list.tolist(), columns=['values'])
    # # # Calculate class weights
    # class_weights = [len(df) / (2 * count) for count in df['values'].value_counts()]
    # # print(class_weights)
...
metrics={
    ...
    'confusion_matrix': ConfusionMatrix(num_classes=nb_class)
}
...
def log_training_results(trainer):
    ...
    metrics = evaluator.state.metrics
    test_acc, test_nll, test_report, test_confusion_matrix = metrics["acc"], metrics["nll"], metrics["class_report"], metrics["confusion_matrix"]
    logger.info(
        "\rEpoch: {}  Train acc: {:.4f} loss: {:.4f}  Val acc: {:.4f} loss: {:.4f}  Test acc: {:.4f} loss: {:.4f}"
        .format(trainer.state.epoch, train_acc, train_nll, val_acc, val_nll, test_acc, test_nll)
    )
    logger.info(test_report)
    logger.info(test_confusion_matrix)
